...
// Locally patched: upstream v10.0.0 declared `expected_update_authority` as
// `&OptionalNonZeroPubkey`, but `spl-token-group-interface` 0.7.1+ changed
// `TokenGroup::update_authority` to `MaybeNull<Pubkey>`. We accept the new
// type and check signer + match equivalently.
fn check_update_authority<T: Copy + Into<Option<Pubkey>>>(
    update_authority_info: &AccountInfo,
    expected_update_authority: &T,
) -> Result<(), ProgramError> {
    if !update_authority_info.is_signer {
        return Err(ProgramError::MissingRequiredSignature);
    }
    let update_authority = (*expected_update_authority)
        .into()
